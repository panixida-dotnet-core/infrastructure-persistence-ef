...
Positional constructors are mapped only for compiler-generated properties declared in the same compilation, including nested records. For customized or inherited positional properties and models from referenced assemblies, assign the sorted values explicitly in `Select`, for example `new Model("") { Name = row.Name.ToUpper() }`. Nullable projections (`IReadModelSorting<Model?>`) use the model's field paths and null keys for null rows. For nullable root projections in EF queries, use reference types: nullable structs are supported in memory, but their SQL translation is limited by EF. Sorting and pagination run after projection; counts also use the projected query, including supported `GroupBy` and `Distinct` projections.
